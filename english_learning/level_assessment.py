"""
영어 수준 파악 테스트 (15문항)
실행: python -m english_learning.level_assessment
"""
import json
import os
...
def _get_level_info(score: int) -> tuple[str, str]:
...
def run_assessment() -> dict:
    print("\n" + "=" * 60)
    print("  📝 영어 수준 파악 테스트 (15문항)")
    print("  각 문항에서 번호를 입력하세요 (1~4)")
    print("=" * 60 + "\n")
    ...
    for i, q in enumerate(QUESTIONS):
        ...
        if correct:
            ...
            print(f"  ❌ 오답 — 정답: {q['answer'] + 1}) {q['options'][q['answer']]}")
            print(f"  💡 {q['explanation']}\n")

        results.append({"question_no": i + 1, "correct": correct})

    level_code, level_label = _get_level_info(score)

    print("\n" + "=" * 60)
    print(f"  🎯 최종 점수: {score} / {len(QUESTIONS)}")
    print(f"  📊 현재 수준: {level_label}")
    print("=" * 60)
    ...
    print(f"\n  💾 프로필 저장 완료: {PROFILE_PATH}")
    print(f"  📬 매일 {VOCAB_COUNTS[level_code]}개의 단어/숙어가 텔레그램으로 발송됩니다.\n")
