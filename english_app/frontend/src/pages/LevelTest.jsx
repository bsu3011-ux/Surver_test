...
export default function LevelTest() {
  ...
  const [selectedOption, setSelectedOption] = useState(null) // -1 = 모르겠다
  ...
  const handleOptionClick = (optionIndex) => {
    ...
    const delay = optionIndex === -1 ? 2000 : 1500  // 모르겠다면 정답 볼 시간 더 줌
  ...
  if (submitError) return (
    <div className="max-w-2xl mx-auto px-4 py-16 text-center">
      ...
      <button onClick={() => submitTest(answers)}
    ...
    <div className="max-w-2xl mx-auto px-4 py-8">
      ...
      <div className="space-y-3 mb-4">
        {q.options.map((option, idx) => {
          let cls = 'w-full text-left p-4 rounded-xl border-2 font-medium transition '
          if (showFeedback) {
            if (idx === q.answer) {
              cls += 'bg-green-50 border-green-400 text-green-800'       // 정답 강조
            } else if (idx === selectedOption) {
              cls += 'bg-red-50 border-red-400 text-red-800'             // 내가 고른 오답
            } else {
              cls += 'border-gray-100 text-gray-300 bg-gray-50 cursor-not-allowed'
            }
          } else {
            cls += 'border-gray-200 hover:border-indigo-400 hover:bg-indigo-50 text-gray-700 bg-white cursor-pointer'
          }
          return (
            <button key={idx} onClick={() => handleOptionClick(idx)}
              disabled={showFeedback} className={cls}>
              <span className="font-bold text-gray-400 mr-3">{idx + 1}.</span>
              {option}
              {showFeedback && idx === q.answer && <span className="ml-2 text-green-600">✓ 정답</span>}
              {showFeedback && idx === selectedOption && idx !== q.answer && <span className="ml-2 text-red-500">✗</span>}
            </button>
          )
        })}
      </div>
      ...
      {!showFeedback && (
        <button
          ...
          className="w-full text-center p-3 rounded-xl border-2 border-dashed border-gray-300 text-gray-400 text-sm hover:border-gray-400 hover:text-gray-500 transition"
        >
          🤔 모르겠다 (정답 보기)
        </button>
      )}
